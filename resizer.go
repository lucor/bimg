...
func resizer(buf []byte, o Options) ([]byte, error) {
	...
	var err error
	if o.ReadAllFrames {
		image, imageType, err = loadImageAllFrames(buf)
	} else {
	...
	supportsShrinkOnLoad = supportsShrinkOnLoad && image.Xsize > 1 && image.Ysize > 1
...
func loadImageAllFrames(buf []byte) (*C.VipsImage, ImageType, error) {
	if len(buf) == 0 {
...
func applyEffects(image *C.VipsImage, o Options) (*C.VipsImage, error) {
	...
	if o.Sharpen.Sigma > 0 && o.Sharpen.Y2 > 0 || o.Sharpen.Y3 > 0 {
